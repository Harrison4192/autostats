...
```{r cache=TRUE}
iris %>% 
  plot_variable_contributions(target = Species)
```

```{r cache=TRUE}
iris %>% 
  get_model_accuracy(Species)
```
...
iris_binary %>% 
  get_model_accuracy(target = Species)
```
...
```{r cache=F}
iris %>% 
  plot_variable_contributions(target = Petal.Length)
```


```{r  cache=TRUE}
iris %>% 
  get_model_accuracy(target = Petal.Length)
```
