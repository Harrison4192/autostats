...
```{r}
iris_train %>%
  auto_tune_xgboost(formula = petal_form, n_iter = 1L, method = "bayes") -> xgb_tuned_bayes
...
```{r}
iris_train %>%
  auto_tune_xgboost(formula = petal_form, n_iter = 7L, method = "grid") -> xgb_tuned_grid

xgb_tuned_grid %>%
  fit(iris_train) %>% 
  hardhat::extract_fit_engine() -> xgb_tuned_fit_grid
